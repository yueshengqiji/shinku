...
**规矩六再次命中。** `public_guard.py` 与 `native_tool_schema.py` 在 Akane 侧都是转发壳
（只有几行 `from code_shared.xxx import *`），若拿 Akane 同路径当对照物，整文件比值会给出
...
- **`evidence_guard.py`（T3）**：与 Akane 侧**逐字节相同**（sha 均为 `6434e377080a`），
